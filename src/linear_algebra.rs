...
impl<const R: usize, const C: usize, T> Matrix<R, C, T> {
    pub fn get_row(&self, r: usize) -> Vec<&T> {
        (0..C).map(|c| &self.data[r * C + c]).collect()
    }

    pub fn get_col(&self, c: usize) -> Vec<&T> {
        (0..R).map(|r| &self.data[r * C + c]).collect()
    }
}
...
impl<const R: usize, const C: usize, T> Matrix<R, C, T> {
        ...
        T: Clone,
    {
        ...
        for c in 0..C {
            for r in 0..R {
                transposed_data.push(self.data[r * C + c].clone());
            }
        }
        ...
        T: Clone,
    {
        ...
        for r in 0..R {
            for c in 0..C {
                ...
                submatrix_data.push(self.data[r * C + c].clone());
            }
        }
        Matrix::new(submatrix_data)
    }
}
